...
'.$inner.'
    ...
    private static function footer() {
        ...
        $text = self::setting('mail_footer', __('This email was sent automatically. Please do not reply.','quotepress'));
    ...
    private static function contact_block() {
        ...
        return '<tr><td style="padding:0 36px 24px;">
          <div style="background:#f8f8f8;border-radius:8px;padding:18px 22px;">
            <p style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#666;">'.esc_html__('Contact Information','quotepress').'</p>
    ...
    private static function items_table_simple( $items ) {
        ...
        foreach ($items as $i => $item) {
            ...
            $rows .= '<tr style="background:'.$bg.';">
              ...
              <td style="padding:9px 12px;border-bottom:1px solid '.$c['border'].';text-align:center;font-weight:700;color:'.$c['primary'].';font-size:13px;">'.esc_html($item['qty']??1).' '.esc_html__('pcs','quotepress').'</td>
            </tr>';
        }
        return '<table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid '.$c['border'].';border-radius:7px;overflow:hidden;font-size:13px;">
          <thead><tr style="background:'.$c['light'].';">
            <th style="padding:9px 12px;text-align:left;color:'.$c['dark'].';font-size:11px;font-weight:700;border-bottom:1px solid '.$c['border'].';">'.esc_html__('Product','quotepress').'</th>
            ...
            <th style="padding:9px 12px;text-align:center;color:'.$c['dark'].';font-size:11px;font-weight:700;border-bottom:1px solid '.$c['border'].';">'.esc_html__('Qty','quotepress').'</th>
          </tr></thead>
          <tbody>'.$rows.'</tbody>
        </table>';
    }
    ...
    private static function items_table_priced( $items, $currency, $vat_rate, $subtotal, $vat_amount, $grand_total ) {
        ...
        $vat_row = intval($vat_rate) > 0
            ? '<tr><td colspan="4" style="text-align:right;padding:7px 12px;color:#666;font-size:13px;">'.esc_html__('VAT','quotepress').' ('.intval($vat_rate).'%)</td>
        ...
        return '<table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid '.$c['border'].';border-radius:8px;overflow:hidden;">
          <thead><tr style="background:'.$c['primary'].';">
            <th style="padding:9px 12px;text-align:left;color:#fff;font-size:11px;font-weight:700;">'.esc_html__('Product','quotepress').'</th>
            <th style="padding:9px 12px;text-align:left;color:#fff;font-size:11px;font-weight:700;">'.esc_html__('Model','quotepress').'</th>
            <th style="padding:9px 12px;text-align:center;color:#fff;font-size:11px;font-weight:700;">'.esc_html__('Qty','quotepress').'</th>
            <th style="padding:9px 12px;text-align:right;color:#fff;font-size:11px;font-weight:700;">'.esc_html__('Unit Price','quotepress').'</th>
            <th style="padding:9px 12px;text-align:right;color:#fff;font-size:11px;font-weight:700;">'.esc_html__('Total','quotepress').'</th>
          </tr></thead>
          <tbody>'.$rows.'</tbody>
          <tfoot style="background:'.$c['light'].';">
            <tr><td colspan="4" style="text-align:right;padding:8px 12px;color:#666;font-size:13px;">'.esc_html__('Subtotal','quotepress').'</td>
            ...
            <tr style="background:'.$c['primary'].';">
              <td colspan="4" style="text-align:right;padding:10px 12px;color:#fff;font-size:14px;font-weight:700;">'.esc_html__('GRAND TOTAL','quotepress').'</td>
    ...
    public static function send_notification( $request, $items ) {
        ...
        $extra_row  = $request->extra_option ? '<p style="margin:10px 0 0;background:#fff8e8;border:1px solid #f0d080;border-radius:7px;padding:10px 14px;font-size:13px;color:#5a4000;"><strong>'.esc_html(QuotePress_Settings::get('extra_option_label',__('Option','quotepress'))).':</strong> '.esc_html($request->extra_option).'</p>' : '';

        $body = self::wrap(
            self::header(
                sprintf( __('New Quote Request: %s', 'quotepress'), $request->project_name ),
                sprintf( __('From %s', 'quotepress'), $request->company_name )
            )
            .'<tr><td style="padding:24px 36px;">
              ...
              <div style="margin-top:20px;text-align:center;">
                <a href="'.esc_url($panel_url).'" style="display:inline-block;background:'.$c['primary'].';color:#fff;text-decoration:none;padding:12px 28px;border-radius:8px;font-size:14px;font-weight:700;">
                  📄 '.esc_html__('Open Panel & Respond','quotepress').'
                </a>
              </div>
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:20px;font-size:13px;">
                <tr><td style="color:#aaa;width:150px;">'.esc_html__('Company','quotepress').'</td><td style="font-weight:600;">'.esc_html($request->company_name).'</td></tr>
                <tr><td style="color:#aaa;">'.esc_html__('Contact','quotepress').'</td><td>'.esc_html($request->contact_name).'</td></tr>
                <tr><td style="color:#aaa;">'.esc_html__('Email','quotepress').'</td><td><a href="mailto:'.esc_attr($request->email).'" style="color:'.$c['primary'].';">'.esc_html($request->email).'</a></td></tr>
                '.($request->phone ? '<tr><td style="color:#aaa;">'.esc_html__('Phone','quotepress').'</td><td>'.esc_html($request->phone).'</td></tr>' : '').'
              </table>
            </td></tr>'
            .self::footer()
        );
        ...
        foreach ( array_map('trim', explode(',', $recipient)) as $to ) {
            if ( is_email($to) ) {
                self::send( $to,
                    sprintf( __('[QuotePress] New Request: %s – %s', 'quotepress'), $request->project_name, $request->company_name ),
                    $body,
    ...
    public static function send_confirmation( $request, $items ) {
        $c     = self::colors();
        $table = self::items_table_simple($items);
        $extra = $request->extra_option ? '<p style="margin:10px 0 0;font-size:13px;color:#555;">'.esc_html(QuotePress_Settings::get('extra_option_label',__('Option','quotepress'))).': <strong style="color:'.$c['primary'].';">'.esc_html($request->extra_option).'</strong></p>' : '';

        $body = self::wrap(
            self::header( __('Your Quote Request Has Been Received', 'quotepress') )
            .'<tr><td style="padding:28px 36px 20px;">
              <p style="margin:0 0 13px;font-size:16px;color:#222;">'.esc_html__('Dear','quotepress').' <strong>'.esc_html($request->contact_name).'</strong>,</p>
              <p style="margin:0 0 13px;font-size:14px;color:#555;line-height:1.8;">
                '.sprintf( esc_html__('Your quote request for project "%s" has been successfully submitted. Our team will review your request and respond with a price quote as soon as possible.', 'quotepress'), esc_html($request->project_name) ).'
              </p>
              <div style="background:'.$c['light'].';border:1px solid '.$c['border'].';border-radius:8px;padding:18px 20px;margin:18px 0;">
                <p style="margin:0 0 12px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:'.$c['primary'].';">'.esc_html__('Your Request Summary','quotepress').'</p>
                '.$table.$extra.'
              </div>
            </td></tr>'
            .self::contact_block()
        ...
        self::send( $request->email,
            sprintf( __('[QuotePress] Request Received – %s', 'quotepress'), $request->project_name ),
            $body
        );
    }

    /* ── Quote email to requester (with PDF) ────────────────── */
    public static function send_quote( $request, $payload, $note, $pdf_path = '' ) {
        ...
        $body = self::wrap(
            '<tr><td style="background:'.$c['primary'].';padding:22px 36px;">
              <table width="100%"><tr>
                ...
                <td style="text-align:right;">
                  <div style="font-size:15px;font-weight:700;color:#fff;">'.esc_html__('PRICE QUOTATION','quotepress').'</div>
                  <div style="font-size:12px;color:rgba(255,255,255,.8);margin-top:3px;">'.esc_html__('No','quotepress').': '.esc_html($quote_no).'</div>
            ...
            <tr><td style="background:'.$c['light'].';padding:13px 36px;border-bottom:1px solid '.$c['border'].';">
              <table width="100%"><tr>
                <td><span style="font-size:12px;color:#777;">'.esc_html__('Attn','quotepress').'.</span><br>
                ...
                <td style="text-align:right;"><span style="font-size:12px;color:#777;">'.esc_html__('Project','quotepress').'</span><br>
                    <strong style="font-size:14px;color:'.$c['primary'].';">'.esc_html($request->project_name).'</strong></td>
              </tr></table>
            </td></tr>
            <tr><td style="padding:22px 36px;">
              '.$table.$note_blk.'
              '.($validity ? '<p style="margin-top:14px;font-size:12px;color:#aaa;">'.sprintf( esc_html__('This quote is valid for %d days.','quotepress'), intval($validity) ).'</p>' : '').'
            </td></tr>'
            .self::contact_block()
            .self::footer()
        );
        ...
        wp_mail(
            $request->email,
            sprintf( __('[QuotePress] Your Price Quotation – %s (%s)', 'quotepress'), $request->project_name, $quote_no ),
            $body,
            $headers,
